...
def perform_feature_engineering_v2(df, is_combined=False):
    """Feature engineering with leakage-safe lag/rolling features and minimal row loss."""
    df = df.copy()
    ...
    final_df = df.dropna(subset=required_cols).copy()
...
def process_all_datasets():
    ...
    for f in individual_files + combined_files:
        ...
        try:
            df = pd.read_csv(f)
            fe_df, init_s, final_s, removed = perform_feature_engineering_v2(df, is_combined=is_comb)
