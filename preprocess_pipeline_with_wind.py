...
OUTPUT_DIR = r'c:\Users\annam\OneDrive\Desktop\ML_Project\DATASETS_CLEANED_WIND'
INDIVIDUAL_DIR = os.path.join(OUTPUT_DIR, 'Individual')
COMBINED_DIR = os.path.join(OUTPUT_DIR, 'Combined')

# Target columns
TARGET_COLS = ['DateTime', 'NO', 'NO2', 'NOx', 'SO2', 'PM2.5', 'PM10', 'CO', 'NH3', 'WS', 'WD']
...
def infer_pollutant(col_name):
    ...
    if norm.startswith('no'):
        return 'NO'
    if norm.startswith('ws') or 'speed' in norm:
        return 'WS'
    if norm.startswith('wd') or 'direction' in norm:
        return 'WD'
...
def clean_file(file_path, location, frequency):
    ...
    try:
        ...
        # Convert pollutants and wind data to numeric
        all_numeric_cols = [c for c in df.columns if c in TARGET_COLS and c != 'DateTime']
...
def process_all_files():
    ...
    for (loc, freq), dfs in aggregates.items():
        ...
        # Apply Clipping [0, 500] for Pollutants only
        ...
        # Wind Direction is inherently [0, 360], leave as is. Wind Speed is rarely > 100 m/s. 
        # So we clip them to reasonable bounds.
        if 'WD' in master_site.columns:
            master_site['WD'] = master_site['WD'] % 360  # Normalize directions just in case
        if 'WS' in master_site.columns:
            master_site['WS'] = master_site['WS'].clip(lower=0, upper=100)
        ...
        # Flag gaps before imputation (we use all num cols here, including wind)
        all_num_cols = [c for c in master_site.columns if c in TARGET_COLS and c != 'DateTime']
        missing_before = master_site[all_num_cols].isna().any(axis=1)
        ...
        master_site[all_num_cols] = master_site[all_num_cols].interpolate(
            method='time',
            limit=3,
...
if __name__ == "__main__":
    process_all_files()
    print("\nPipeline Complete! Check DATASETS_CLEANED_WIND folder.")
